...
/**
 * Thread-safe linked list implementation with fail-fast iterators.
 */
class VersionedSafeLinkedList<T> : Iterable<T> {
    ...
    override fun iterator(): Iterator<T> = object : Iterator<T> {
        ...
        private val listSentinel: Node<T>
        private val expectedVersionNumber: Long
        ...
        private fun checkForModification() {
            if (guard.withLock { versionNumber } != expectedVersionNumber) {
         ...
         */
        override fun hasNext(): Boolean = guard.withLock {
            checkForModification()
            current !== listSentinel
        }
    }
}
